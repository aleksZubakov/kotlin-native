...
open class KlibMergerMetadataSerializer(
    ...
    fun serialize(environment: KotlinCoreEnvironment) {
        ...
        analyzer.analyzeAndReport(files) {
            KlibMergerCommonAnalyzerFacade.analyzeFiles(files, moduleName, dependOnOldBuiltIns, configuration.languageVersionSettings) { content ->
    ...
    private fun getPackageFilePath(packageFqName: FqName, fileName: String): String =
            packageFqName.asString().replace('.', '/') + "/" +
                    PackagePartClassUtils.getFilePartShortName(fileName) + MetadataPackageFragment.DOT_METADATA_FILE_EXTENSION

    private fun getClassFilePath(classId: ClassId): String =
            classId.asSingleFqName().asString().replace('.', '/') + MetadataPackageFragment.DOT_METADATA_FILE_EXTENSION
